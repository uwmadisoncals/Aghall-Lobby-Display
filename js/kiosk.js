...
function arrivalsCallback(jsondata) {
    if( jsondata.status == "-1") {
        ...
        $("#"+stopID+"-estimates").replaceWith('<div id="'+stopID+'-estimates" class="estimates"><div class="title"><span class="direction">'+metro_direction[stopID]+'</span> buses at <span id="location">' + metro_location[stopID] + '</span> <span class="estimate">(' + timestamp + ' estimate)</span></div>');
        ...
        for( var i=0; i < routes.length; i++ ) {
          	...
          	destination = destination.substring(0,22); //limit the length in case of long ones (unknown risk)
          	if (minutes<6) {
